...
vi.mock('./router.js', async () => {
    ...
    return {
        ...actual,
        findChannel: vi.fn(),
        formatMessages: vi.fn((msgs: NewMessage[]) => msgs.map(m => m.content).join('\n')),
    };
});
...
// Now import
import { routeNewMessages, batchTimers, _setRegisteredGroups, queue, channels } from './index.js';
...
describe('Message Batching', () => {
    ...
    it('batches multiple messages and only sends once after quiet period', async () => {
        const msg1 = { id: '1', chat_jid: chatJid, content: '@Andy hello', timestamp: '1000' } as NewMessage;
        const msg2 = { id: '2', chat_jid: chatJid, content: 'how are you?', timestamp: '2000' } as NewMessage;

        // Mock DB: initially empty, then both messages
        (db.getMessagesSince as any).mockReturnValueOnce([]).mockReturnValue([msg1, msg2]);
